...
Локальный operator toolkit для controlled job search через официальный
публичный API HeadHunter. Приложение выполняет настроенные поиски, сохраняет
вакансии в SQLite, загружает подробности, рассчитывает прозрачный rule-based
score, создаёт автономные HTML/CSV/JSONL-экспорты и предоставляет локальный
web UI для daily workflow, очереди, cockpit и операторских действий.
...
`pytest` включён в `requirements.txt` как часть локального release baseline.
...
Правила намеренно простые и объяснимые. На текущем release baseline этапе LLM
не используется.
